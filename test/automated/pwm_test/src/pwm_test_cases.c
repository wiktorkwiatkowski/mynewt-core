...
test_pwm_duty_cycle(int duty_percent)
{
    ...
    MTEST_CASE_ASSERT(rc == 0, "measurement timeout for duty %d%%",
                      duty_percent);
    ...
    MTEST_CASE_ASSERT(diff < MYNEWT_VAL(PWM_TOLERANCE),
                      "duty tolerance exceeded: expected %d%%, measured %d%%, diff %d%% (max %d%%)",
                      duty_percent, measured_duty, diff,
                      MYNEWT_VAL(PWM_TOLERANCE));
}
